...
async function startServer() {
  ...
  app.post('/api/db/backups/trigger', async (req, res) => {
    ...
    if (backupType === 'pg_dump') {
      ...
      fullCommand = `sshpass -p '${passEscaped}' ssh ${sshOpts} ${portFlag}${user}@${host} "mkdir -p ${folder} && sudo pg_dump -U ${dbUser} ${databaseName} -F p --clean --if-exists > ${finalLocation}"`;
    } else {
      const folderName = `basebackup_${databaseName}_${timestamp}`;
      finalLocation = `${folder}/${folderName}`;
      fullCommand = `sshpass -p '${passEscaped}' ssh ${sshOpts} ${portFlag}${user}@${host} "mkdir -p ${finalLocation} && sudo pg_basebackup -h localhost -p 5432 -U ${dbUser} -D ${finalLocation} -Fp -P"`;
    }
